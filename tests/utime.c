...
{
	struct linux_utimbuf_t const t =
	{
		.actime = 1234,
		.modtime = 1234,
	};
	if (linux_utime("some non existant file", &t) != linux_ENOENT)
...
int main(void)
{
	...
	printf("Start testing utime.\n");
	DO_TEST(invalid_file, &ret);
	DO_TEST(correct_usage, &ret);
	printf("Finished testing utime.\n");
