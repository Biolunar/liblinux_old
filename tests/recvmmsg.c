...
static enum TestResult test_invalid_fd(void)
{
	...
	if (linux_recvmmsg(linux_stderr + 1, &msg, 1, 0, 0, 0) != linux_EBADF)
...
static enum TestResult test_segfault(void)
{
	...
	if (linux_recvmmsg(con_fd, &msg, 1, 0, 0, 0) != linux_EFAULT)
...
static enum TestResult test_not_connected(void)
{
	...
	if (linux_recvmmsg(fd, &msg, 1, 0, 0, 0) != linux_ENOTCONN)
	{
		linux_close(fd);
		return TEST_RESULT_FAILURE;
	}
...
static enum TestResult test_fd_not_socket(void)
{
	...
	if (linux_recvmmsg(linux_stdout, &msg, 1, 0, 0, 0) != linux_ENOTSOCK)
		return TEST_RESULT_FAILURE;
...
static enum TestResult test_correct_usage(void)
{
	...
	if (linux_recvmmsg(con_fd, &msg, 1, 0, 0, &size) || size != sizeof message || strncmp(message, buf, sizeof message))
		ret = TEST_RESULT_FAILURE;
