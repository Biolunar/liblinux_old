...
static enum TestResult test_segfault(void)
{
	...
	if (linux_execveat(linux_AT_FDCWD, 0, argv_ok, envp_ok, 0) != linux_EFAULT)
		return TEST_RESULT_FAILURE;

	if (linux_execveat(linux_AT_FDCWD, progname, argv_bad, envp_ok, 0) != linux_EFAULT)
		return TEST_RESULT_FAILURE;

	if (linux_execveat(linux_AT_FDCWD, progname, argv_ok, envp_bad, 0) != linux_EFAULT)
		return TEST_RESULT_FAILURE;
...
static enum TestResult test_correct_usage(void)
{
	...
	else if (!pid) // child
	{
		...
		linux_execveat(linux_AT_FDCWD, progname, argv, envp, 0);
...
int main(int argc, char* argv[])
{
	...
	printf("Start testing execveat.\n");
	DO_TEST(segfault, &ret);
	DO_TEST(correct_usage, &ret);
	printf("Finished testing execveat.\n");
