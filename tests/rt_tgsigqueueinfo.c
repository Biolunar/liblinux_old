...
static enum TestResult test_invalid_pids(void)
{
	linux_pid_t pid;
	if (linux_getpid(&pid))
	...
	if (linux_rt_tgsigqueueinfo(pid, (linux_pid_t)-1, linux_SIGUSR1, &info) != linux_EINVAL || g_success == true)
	...
	if (linux_rt_tgsigqueueinfo((linux_pid_t)-1, tid, linux_SIGUSR1, &info) != linux_EINVAL || g_success == true)
		return TEST_RESULT_FAILURE;
...
static enum TestResult test_correct_usage(void)
{
	...
	if (linux_rt_tgsigqueueinfo(pid, tid, linux_SIGUSR1, &info) || g_success == false)
		return TEST_RESULT_FAILURE;
...
int main(void)
{
	...
	printf("Start testing rt_tgsigqueueinfo.\n");
	DO_TEST(invalid_pids, &ret);
	DO_TEST(correct_usage, &ret);
	printf("Finished testing rt_tgsigqueueinfo.\n");
