...
static enum TestResult test_invalid_fd(void)
{
	...
	struct linux_user_msghdr_t msg =
	{
		.msg_iov = &vec,
		.msg_iovlen = 1,
	};

	if (linux_recvmsg(linux_stderr + 1, &msg, 0, 0) != linux_EBADF)
...
static enum TestResult test_segfault(void)
{
	...
	if (linux_recvmsg(con_fd, &msg, 0, 0) != linux_EFAULT)
...
static enum TestResult test_not_connected(void)
{
	...
	if (linux_recvmsg(fd, &msg, 0, 0) != linux_ENOTCONN)
	{
		linux_close(fd);
		return TEST_RESULT_FAILURE;
	}
...
static enum TestResult test_fd_not_socket(void)
{
	...
	if (linux_recvmsg(linux_stdout, &msg, 0, 0) != linux_ENOTSOCK)
		return TEST_RESULT_FAILURE;
...
static enum TestResult test_correct_usage(void)
{
	...
	if (linux_recvmsg(con_fd, &msg, 0, &size) || size != sizeof message || strncmp(message, buf, sizeof message))
		ret = TEST_RESULT_FAILURE;
