...
{
	if (linux_getrlimit(linux_RLIMIT_NOFILE, 0) != linux_EFAULT)
...
static enum TestResult test_invalid_resource(void)
{
	...
	if (linux_getrlimit(linux_RLIM_NLIMITS, &rlim) != linux_EINVAL)
		return TEST_RESULT_FAILURE;
...
int main(void)
{
	...
	printf("Start testing getrlimit.\n");
	DO_TEST(segfault, &ret);
	DO_TEST(invalid_resource, &ret);
	DO_TEST(correct_usage, &ret);
	printf("Finished testing getrlimit.\n");
