...
static enum TestResult test_invalid_fd(void)
{
	...
	if (linux_preadv(linux_stderr + 1, &vec, 1, 0, 0, 0) != linux_EBADF)
...
static enum TestResult test_invalid_buf(void)
{
	...
	if (linux_preadv((linux_fd_t)fd, &vec, 1, 0, 0, 0) != linux_EFAULT)
	{
		linux_close(fd);
		return TEST_RESULT_FAILURE;
	}
...
static enum TestResult test_read_zero(void)
{
	...
	size_t result = 0;
	if (linux_preadv((linux_fd_t)fd, &vec, 1, 0, 0, &result) || result)
	{
		linux_close(fd);
		return TEST_RESULT_FAILURE;
	}
