...
static enum TestResult test_invalid_mqd(void)
{
	...
	struct linux_sigevent_t const event =
	{
		.sigev_notify = linux_SIGEV_NONE,
		.sigev_un.tid = tid,
	};
	if (linux_mq_notify(0, &event) != linux_EBADF)
...
int main(void)
{
	...
	printf("Start testing mq_notify.\n");
	DO_TEST(invalid_mqd, &ret);
	DO_TEST(correct_usage, &ret);
	printf("Finished testing mq_notify.\n");
