...
{
	if (linux_lchown(0, 0, 0) != linux_EFAULT)
...
{
	if (linux_lchown(filename, 0, 0) != linux_ENOENT)
		return TEST_RESULT_FAILURE;
...
static enum TestResult test_correct_usage(void)
{
	...
	if (linux_lchown(filename, uid, gid))
	{
...
int main(void)
{
	...
	printf("Start testing lchown.\n");
	DO_TEST(segfault, &ret);
	DO_TEST(invalid_filename, &ret);
	DO_TEST(correct_usage, &ret);
	printf("Finished testing lchown.\n");
