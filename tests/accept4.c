...
{
	if (linux_accept4(linux_stderr + 1, 0, 0, 0, 0) != linux_EBADF)
...
static enum TestResult test_segfault(void)
{
	...
	if (linux_accept4(fd, (struct linux_sockaddr_t*)1, 0, 0, 0) != linux_EFAULT)
	{
		linux_close(fd);
		return TEST_RESULT_FAILURE;
	}
...
static enum TestResult test_non_listening(void)
{
	...
	if (linux_accept4(fd, (struct linux_sockaddr_t*)1, 0, 0, 0) != linux_EINVAL)
	{
		linux_close(fd);
		return TEST_RESULT_FAILURE;
	}
...
{
	if (linux_accept4(linux_stdout, 0, 0, 0, 0) != linux_ENOTSOCK)
		return TEST_RESULT_FAILURE;
...
static enum TestResult test_correct_usage(void)
{
	...
	int len = 0;
	if (linux_accept4(fd, (struct linux_sockaddr_t*)&csa, &len, linux_SOCK_CLOEXEC, 0))
	{
