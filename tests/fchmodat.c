...
{
	if (linux_fchmodat(linux_AT_FDCWD, 0, mode) != linux_EFAULT)
...
{
	if (linux_fchmodat(linux_AT_FDCWD, filename, mode) != linux_ENOENT)
		return TEST_RESULT_FAILURE;
...
static enum TestResult test_correct_usage(void)
{
	...
	if (linux_fchmodat(linux_AT_FDCWD, filename, new_mode))
	{
...
int main(void)
{
	...
	printf("Start testing fchmodat.\n");
	DO_TEST(segfault, &ret);
	DO_TEST(invalid_filename, &ret);
	DO_TEST(correct_usage, &ret);
	printf("Finished testing fchmodat.\n");
