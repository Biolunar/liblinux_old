...
static enum TestResult test_segfault(void)
{
	...
	long ret;
	if (linux_io_getevents(context, 1, 1, 0, (struct linux_timespec_t*)-1, &ret) != linux_EFAULT)
	{
...
static enum TestResult test_invalid_nr(void)
{
	...
	long ret;
	if (linux_io_getevents(context, -1, 1, 0, 0, &ret) != linux_EINVAL)
	{
		linux_io_destroy(context);
		return TEST_RESULT_FAILURE;
	}
...
int main(void)
{
	...
	printf("Start testing io_getevents.\n");
	DO_TEST(segfault, &ret);
	DO_TEST(invalid_nr, &ret);
	DO_TEST(correct_usage, &ret);
	printf("Finished testing io_getevents.\n");
