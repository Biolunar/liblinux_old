...
{
	if (linux_getsockname(linux_stderr + 1, 0, 0) != linux_EBADF)
...
{
	if (linux_getsockname(linux_stderr, 0, 0) != linux_ENOTSOCK)
		return TEST_RESULT_FAILURE;
...
static enum TestResult test_segfault(void)
{
	...
	if (linux_getsockname(fd, 0, &size) != linux_EFAULT)
	{
		linux_close(fd);
		return TEST_RESULT_FAILURE;
	}
...
static enum TestResult test_correct_usage(void)
{
	...
	if  (linux_getsockname(fd, (struct linux_sockaddr_t*)&csa, &csa_len) || memcmp(&sa, &csa, sizeof sa) || csa_len != sizeof csa)
		return TEST_RESULT_FAILURE;
