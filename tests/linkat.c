...
{
	if (linux_linkat(linux_AT_FDCWD, 0, linux_AT_FDCWD, filename, 0) != linux_EFAULT)
		return TEST_RESULT_FAILURE;

	if (linux_linkat(linux_AT_FDCWD, 0, linux_AT_FDCWD, 0, 0) != linux_EFAULT)
		return TEST_RESULT_FAILURE;
...
{
	if (linux_linkat(linux_AT_FDCWD, filename, linux_AT_FDCWD, "some new filename", 0) != linux_ENOENT)
		return TEST_RESULT_FAILURE;
...
static enum TestResult test_correct_usage(void)
{
	...
	if (linux_linkat(linux_AT_FDCWD, old_test_file, linux_AT_FDCWD, new_test_file, 0))
		return TEST_RESULT_FAILURE;
...
int main(void)
{
	...
	printf("Start testing linkat.\n");
	DO_TEST(segfault, &ret);
	DO_TEST(invalid_filename, &ret);
	DO_TEST(correct_usage, &ret);
	printf("Finished testing linkat.\n");
