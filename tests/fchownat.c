...
{
	if (linux_fchownat(linux_AT_FDCWD, 0, 0, 0, 0) != linux_EFAULT)
...
{
	if (linux_fchownat(linux_AT_FDCWD, filename, 0, 0, 0) != linux_ENOENT)
		return TEST_RESULT_FAILURE;
...
static enum TestResult test_correct_usage(void)
{
	...
	if (linux_fchownat(linux_AT_FDCWD, filename, uid, gid, 0))
	{
...
int main(void)
{
	...
	printf("Start testing fchownat.\n");
	DO_TEST(segfault, &ret);
	DO_TEST(invalid_filename, &ret);
	DO_TEST(correct_usage, &ret);
	printf("Finished testing fchownat.\n");
