...
static enum TestResult test_segfault(void)
{
	...
	if (linux_readlinkat(linux_AT_FDCWD, newname, 0, 1, 0) != linux_EFAULT)
	{
...
static enum TestResult test_invalid_size(void)
{
	...
	if (linux_readlinkat(linux_AT_FDCWD, newname, buf, -1, 0) != linux_EINVAL)
	{
		linux_unlink(newname);
		return TEST_RESULT_FAILURE;
	}
...
{
	char buf[linux_PATH_MAX];
	if (linux_readlinkat(linux_AT_FDCWD, newname, buf, sizeof buf, 0) != linux_ENOENT)
		return TEST_RESULT_FAILURE;
...
static enum TestResult test_correct_usage(void)
{
	...
	char buf[linux_PATH_MAX];
	if (linux_readlinkat(linux_AT_FDCWD, newname, buf, sizeof buf, 0))
	{
		linux_unlink(newname);
		return TEST_RESULT_FAILURE;
	}
