...
{
	if  (linux_fdatasync(linux_stderr + 1) != linux_EBADF)
...
{
	if  (linux_fdatasync(linux_stdout) != linux_EINVAL)
		return TEST_RESULT_FAILURE;
...
int main(void)
{
	...
	printf("Start testing fdatasync.\n");
	DO_TEST(invalid_fd, &ret);
	DO_TEST(invalid_file, &ret);
	DO_TEST(correct_usage, &ret);
	printf("Finished testing fdatasync.\n");
