...
static enum TestResult test_buffer_too_small(void)
{
	...
	size_t ret;
	if (linux_listxattr(filename, buf, sizeof buf, &ret) != linux_ERANGE)
	{
		linux_unlink(filename);
...
static enum TestResult test_correct_usage(void)
{
	...
	char buf[512];
	size_t ret;
	if (linux_listxattr(filename, buf, sizeof buf, &ret))
	{
		linux_unlink(filename);
		return TEST_RESULT_FAILURE;
	}
...
int main(void)
{
	...
	printf("Start testing listxattr.\n");
	DO_TEST(buffer_too_small, &ret);
	DO_TEST(correct_usage, &ret);
	printf("Finished testing listxattr.\n");
