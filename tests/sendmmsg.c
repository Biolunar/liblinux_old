...
static enum TestResult test_invalid_fd(void)
{
	...
	if (linux_sendmmsg(linux_stderr + 1, &msg, 1, 0, 0) != linux_EBADF)
...
static enum TestResult test_segfault(void)
{
	...
	if (linux_sendmmsg(fd, &msg, 1, 0, 0) != linux_EFAULT)
	{
		linux_close(fd);
		return TEST_RESULT_FAILURE;
	}
...
static enum TestResult test_no_dest_addr(void)
{
	...
	if (linux_sendmmsg(fd, &msg, 1, 0, 0) != linux_EDESTADDRREQ)
	{
		linux_close(fd);
		return TEST_RESULT_FAILURE;
	}
...
static enum TestResult test_fd_not_socket(void)
{
	...
	if (linux_sendmmsg(linux_stdout, &msg, 1, 0, 0) != linux_ENOTSOCK)
		return TEST_RESULT_FAILURE;
...
static enum TestResult test_correct_usage(void)
{
	...
	unsigned int ret = 0;
	if (linux_sendmmsg(fd, &msg, 1, 0, &ret) || ret != 1)
		return TEST_RESULT_FAILURE;
